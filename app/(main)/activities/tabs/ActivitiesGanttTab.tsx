        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: spacing.md }}>
          <div style={{ display: "flex", alignItems: "center", gap: spacing.md }}>
            <div style={{ display: "flex", gap: spacing.xs }}>
              <Button variant="outline" size="sm" onClick={prevPeriod}><ChevronRight size={16}/></Button>
              <Button variant="outline" size="sm" onClick={nextPeriod}><ChevronLeft size={16}/></Button>
            </div>
            <h2 style={{ margin: 0, fontSize: 18 }}>{title}</h2>
          </div>
          
          <div style={{ display: "flex", gap: spacing.sm, alignItems: "center" }}>
             <div style={{ display: "flex", backgroundColor: colors.surfaceAlt, borderRadius: 6, padding: 2 }}>
                <button
                ...
                <button
                  ...
                  style={{
                    ...
                    boxShadow: viewMode === "quarter" ? "0 1px 2px rgba(0,0,0,0.1)" : "none",
             ...
             <div style={{ fontSize: 14, color: colors.textMuted, borderRight: `1px solid ${colors.borderMuted}`, paddingRight: spacing.sm }}>
...
function GanttView({ viewStart, viewEnd, groups, activities, viewMode }: { viewStart: Date, viewEnd: Date, groups: string[], activities: Activity[], viewMode: ViewMode }) {
  ...
  if (viewMode === "month") {
    ...
    columns = eachWeekOfInterval({ start: viewStart, end: viewEnd }, { weekStartsOn: 0 });
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ 
          ...
          {columns.map((colDate, i) => {
            ...
            const showMonthLabel = viewMode === "quarter" && colDate.getDate() <= 7; 
            ...
            return (
              <div key={i} style={{ 
                ...
                backgroundColor: isTodayDate ? "rgba(59, 130, 246, 0.1)" : undefined,
                position: "relative",
                minWidth: 0 // Allow shrinking
              }}>
                <div style={{ fontWeight: 600, color: isTodayDate ? colors.primary : undefined }}>
              ...
              <div key={group} style={{ 
                ...
                {groupActivities.map(activity => {
                  ...
                  if (viewMode === "month") {
                    ...
                    colIndex = differenceInCalendarWeeks(actDate, viewStart, { weekStartsOn: 0 });
                  }
                  ...
                  return (
                    <div 
                      key={activity.id}
                      title={`${activity.kind} - ${format(actDate, "dd/MM/yyyy")}\n${activity.start_time || ""} - ${activity.end_time || ""}\n${activity.location || ""}`}
                      style={{
                        position: "absolute",
...
function getActivityColor(kind: string) {
  ...
  if (kind === "חברתי" || kind === "social") return "#10b981"; // Emerald green
