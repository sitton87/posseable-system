...
import { Section, SmallActionButton, StatusPill } from "@/app/components/shared";
import { colors, spacing, radii } from "@/app/styles/foundations";
import { tableStyle, tableHeaderStyle, tableCellStyle, labelStyle, inputStyle } from "@/app/styles/components";
...
export function StaffCard({ volunteer, onClose }: StaffCardProps) {
  ...
  const [certFile, setCertFile] = useState<{ name: string; mime: string; data: string } | null>(null);

  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      // Fetch assigned roles
      const resAssigned = await fetch(`/api/volunteers/${volunteer.national_id}/roles`);
      ...
      const resDetails = await fetch(`/api/volunteers/${volunteer.national_id}`);
  ...
  const selectedRoleObj = availableRoles.find(r => String(r.id) === selectedRoleId);


  const handleRemoveRole = async (roleId: number) => {
    if (!confirm("Are you sure you want to remove this role assignment?")) return;
    try {
      const res = await fetch(`/api/volunteers/${volunteer.national_id}/roles/${roleId}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (data.success) {
  ...
  return (
    <Modal open={true} onClose={onClose} width="min(800px, 95vw)" style={{ padding: 0, overflow: 'hidden' }}>
      <div style={{ display: 'flex', height: '80vh' }}>
        {/* Sidebar / Header Info */}
        <div style={{ width: 250, background: colors.surface, borderRight: `1px solid ${colors.borderMuted}`, padding: spacing.lg }}>
          <div style={{ marginBottom: spacing.lg }}>
            <div style={{ width: 64, height: 64, borderRadius: '50%', background: colors.primary, color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, fontWeight: 'bold', marginBottom: spacing.md }}>
              {volunteer.full_name.charAt(0)}
            </div>
            <h3 style={{ margin: 0 }}>{volunteer.full_name}</h3>
            <div style={{ color: colors.textMuted, fontSize: 14 }}>{volunteer.classification}</div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.xs }}>
             <button 
                ...
                style={{ 
                  ...
                  background: activeTab === "roles" ? colors.background : 'transparent',
                  border: 'none',
                  cursor: 'pointer',
             ...
             <button 
                ...
                style={{ 
                  ...
                  padding: spacing.sm, 
                  borderRadius: radii.sm, 
                  background: activeTab === "history" ? colors.background : 'transparent',
                  border: 'none',
                  cursor: 'pointer',
          ...
          {activeTab === "roles" && (
             <div>
               ...
               {loading ? (
                 ...
                 <div style={{ color: colors.textMuted }}>אין תפקידים משויכים.</div>
               ) : (
                     ...
                     <div key={role.role_id} style={{ 
                       ...
                       <div>
                         <div style={{ display: 'flex', alignItems: 'center', gap: spacing.sm, marginBottom: spacing.xs }}>
                           <div style={{ width: 12, height: 12, borderRadius: '50%', background: role.color_hex }}></div>
                           <span style={{ fontWeight: 600, fontSize: 16 }}>{role.role_name}</span>
                         </div>
                         <div style={{ fontSize: 13, color: colors.textMuted }}>
                           הוקצה ב: {new Date(role.assigned_at).toLocaleDateString('he-IL')}
                           {role.training_date && (
                             <span style={{ marginRight: 4 }}>
                               • עבר הדרכה: {new Date(role.training_date).toLocaleDateString('he-IL')}
                             </span>
                           )}
                           {role.valid_until && (
                             <span style={{ 
                               color: new Date(role.valid_until) < new Date() ? colors.danger : colors.textMuted,
                               fontWeight: new Date(role.valid_until) < new Date() ? 'bold' : 'normal',
                               marginRight: 4
                             }}>
                               • בתוקף עד: {new Date(role.valid_until).toLocaleDateString('he-IL')}
                               {new Date(role.valid_until) < new Date() && " (פג תוקף!)"}
                             </span>
                           )}
                         </div>
                         {role.notes && <div style={{ marginTop: spacing.xs, fontSize: 14 }}>{role.notes}</div>}
                         {role.certificate_url && ( // Assuming certificate_url might still be used for legacy or if we generate a download link
                            <div style={{ marginTop: spacing.xs }}>
                               <a href={role.certificate_url} target="_blank" rel="noreferrer" style={{ fontSize: 13, color: colors.primary }}>
                                 📄 צפה בתעודה
                               </a>
                            </div>
                         )}
                       ...
                       <div style={{ display: 'flex', gap: spacing.xs }}>
                           <SmallActionButton variant="secondary" onClick={() => handleEditRole(role)}>
                             ערוך
                           </SmallActionButton>
                           <SmallActionButton variant="secondary" style={{ color: colors.danger }} onClick={() => handleRemoveRole(role.role_id)}>
                             הסר
                           </SmallActionButton>
                       </div>
                     </div>
          ...
          {activeTab === "history" && (
            <div>
              ...
              {loading ? (
                ...
                <div style={{ color: colors.textMuted }}>אין פעילות מתועדת.</div>
              ) : (
                      ...
                      {activities.map(a => (
                        <tr key={a.activity_id}>
                          <td style={tableCellStyle}>
                            {a.activity_date ? new Date(a.activity_date).toLocaleDateString('he-IL') : '—'}
                          </td>
      ...
      {/* Assign Role Modal */}
      {showAssignModal && (
        <Modal open={true} onClose={() => setShowAssignModal(false)} width="400px">
          <h3>{editingRoleId ? "עריכת תפקיד" : `הוספת תפקיד ל${volunteer.full_name}`}</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.md, marginTop: spacing.md }}>
            ...
            {selectedRoleObj?.requires_training && (
                <div style={{ background: colors.warningSoft, padding: spacing.sm, borderRadius: radii.sm, fontSize: 13, color: colors.warningText, marginBottom: spacing.xs }}>
            ...
            {(selectedRoleObj?.requires_training || trainingDate) && (
                <div>
                  <label style={labelStyle}>תאריך הדרכה {selectedRoleObj?.requires_training && "*"}</label>
                  <input 
                    type="date" 
                    style={inputStyle} 
                    value={trainingDate} 
            ...
            {selectedRoleObj?.requires_renewal && (
                <div style={{ background: colors.warningSoft, padding: spacing.sm, borderRadius: radii.sm, fontSize: 13, color: colors.warningText }}>
            ...
            {(selectedRoleObj?.requires_renewal || validUntil) && (
                <div>
                  <label style={labelStyle}>תוקף עד {selectedRoleObj?.requires_renewal && "*"}</label>
                  <input 
                    type="date" 
                    style={inputStyle} 
                    value={validUntil} 
            ...
            <div>
              ...
              />
              {certFile && <div style={{ fontSize: 12, marginTop: 4 }}>קובץ נבחר: {certFile.name}</div>}
            </div>
            <div>
